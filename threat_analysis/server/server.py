...
# Assuming default initialization is sufficient
...
def update_diagram():
    ...
    try:
        ...
        if not threat_model:
            return jsonify({"error": "Failed to create threat model"}), 500
        ...
        if not dot_code:
            return (
                jsonify({"error": "Failed to generate DOT code from model"}),
                500,
            )
        ...
        svg_path = diagram_generator.generate_diagram_from_dot(
        ...
        full_html = diagram_generator._create_complete_html(
...
def export_files():
    ...
    try:
        ...
        if not threat_model:
            return (
                jsonify({"error": "Failed to create or validate threat model"}),
                500,
            )
        ...
        if export_format == "svg":
            ...
            generated_path = diagram_generator.generate_diagram_from_dot(
            ...
            diagram_generator.generate_diagram_from_dot(
            ...
            diagram_generator._generate_html_with_legend(
        ...
        logging.error(
            f"Error during export for format {export_format}: {e}",
            exc_info=True,
        )
        return (
            jsonify({"error": f"An error occurred during export: {str(e)}"}),
            500,
        )
...
def export_all_files():
    ...
    try:
        ...
        report_generator.generate_json_analysis(
        ...
        return (
            jsonify({"error": f"An error occurred during export all: {str(e)}"}),
            500,
        )